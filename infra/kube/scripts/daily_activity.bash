#!/bin/bash
# Имитация дневной активности для ДЗ «Мониторинг и логирование».
# Требует: backend доступен на API_URL (по умолчанию localhost:4111), curl, jq.
# Перед запуском: kubectl port-forward svc/backend 4111:4111 -n default
...
# Извлечь поле "id" из JSON без jq (работает на Windows/Git Bash)
get_json_id() {
  local json="$1"
